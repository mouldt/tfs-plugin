...
public class TeamFoundationServerScmTest {
    ...
    @Test public void assertVersionSpecResolvesBuildVariables() {
        ParametersAction action = mock(ParametersAction.class);
        when(action.substitute(isA(AbstractBuild.class), isA(String.class))).thenReturn("VS-RESOLVED");
        AbstractBuild build = mock(AbstractBuild.class);
        when(build.getAction(ParametersAction.class)).thenReturn(action);

        TeamFoundationServerScm scm = new TeamFoundationServerScm(null, null, ".", false, "", "user", "password", "VS-${PARAM}");
        assertEquals("The version spec wasnt resolved", "VS-RESOLVED", scm.getVersionSpec(build));
    }    
    ...
    @Test
    public void assertEmptyVersionSpecIsOK() throws Exception {
        hudson.util.FormValidation val = TeamFoundationServerScm.DescriptorImpl.checkVersionSpec("");
        assertEquals("Empty version spec failed.", hudson.util.FormValidation.Kind.OK, val.kind);
    }
    @Test
    public void assertDateVersionSpecIsOK() throws Exception {
        hudson.util.FormValidation val = TeamFoundationServerScm.DescriptorImpl.checkVersionSpec("D12/31/2005");
        assertEquals("Date version spec failed.", hudson.util.FormValidation.Kind.OK, val.kind);
    }
    @Test
    public void assertLabelVersionSpecIsOK() throws Exception {
        hudson.util.FormValidation val = TeamFoundationServerScm.DescriptorImpl.checkVersionSpec("Labc");
        assertEquals("Date version spec failed.", hudson.util.FormValidation.Kind.OK, val.kind);
    }
    @Test
    public void assertLatestVersionSpecIsOK() throws Exception {
        hudson.util.FormValidation val = TeamFoundationServerScm.DescriptorImpl.checkVersionSpec("T");
        assertEquals("Latest version spec failed.", hudson.util.FormValidation.Kind.OK, val.kind);
    }
    @Test
    public void assertWorkspaceVersionSpecIsOK() throws Exception {
        hudson.util.FormValidation val = TeamFoundationServerScm.DescriptorImpl.checkVersionSpec("Wabc");
        assertEquals("Workspace version spec failed.", hudson.util.FormValidation.Kind.OK, val.kind);
    }
    @Test
    public void assertDateVersionSpecErrors() throws Exception {
        hudson.util.FormValidation val = TeamFoundationServerScm.DescriptorImpl.checkVersionSpec("D31/32/2005");
        assertEquals("Date version spec error failed.", hudson.util.FormValidation.Kind.ERROR, val.kind);
    }
    @Test
    public void assertChangesetVersionSpecIsOK() throws Exception {
        hudson.util.FormValidation val = TeamFoundationServerScm.DescriptorImpl.checkVersionSpec("C1234");
        assertEquals("Date version spec failed.", hudson.util.FormValidation.Kind.OK, val.kind);
    }
    @Test
    public void assertChangesetVersionSpecErrors() throws Exception {
        hudson.util.FormValidation val = TeamFoundationServerScm.DescriptorImpl.checkVersionSpec("CABC");
        assertEquals("Date version spec failed.", hudson.util.FormValidation.Kind.ERROR, val.kind);
    }
    @Test
    public void assertInvalidVersionSpecErrors() throws Exception {
        hudson.util.FormValidation val = TeamFoundationServerScm.DescriptorImpl.checkVersionSpec("G123");
        assertEquals("Invalid version spec failed.", hudson.util.FormValidation.Kind.ERROR, val.kind);
    }
    @Test
    public void assertVersionSpecLengthErrors() throws Exception {
        hudson.util.FormValidation val = TeamFoundationServerScm.DescriptorImpl.checkVersionSpec("D");
        assertEquals("version spec length failed.", hudson.util.FormValidation.Kind.ERROR, val.kind);
    }
}
